...
function initializeDatabase() {
  ...
  const createTableQuery = `
    CREATE TABLE IF NOT EXISTS tournament_scores (
      ...
      average_level_performance REAL DEFAULT 0,
      ...
      ai_training_accuracy REAL DEFAULT 0,
...
app.post('/api/register-user', (req, res) => {
  const { registrationNumber, studentName, sessionId } = req.body;
  ...
  try {
    ...
    const checkQuery = `SELECT registration_number, final_score, game_start_time FROM tournament_scores WHERE registration_number = ?`;
    ...
    const insertQuery = `
      ...
      (registration_number, student_name, final_score, levels_completed, levels_skipped,
       accuracy_rate, time_remaining, total_time_taken, game_start_time, game_end_time,
       level_breakdown, ip_address, session_id, total_attempts, successful_attempts,
       average_level_performance, level_1_time, level_2_time, level_3_time, level_4_time, level_5_time,
       ...
       ai_training_accuracy, coding_challenge_solved, total_clicks, level_switches, hints_used, errors_made,
       user_agent, screen_resolution, device_type)
      VALUES (?, ?, 0, 0, 0, 0, 600000, 0, NULL, NULL, '[]', ?, ?, 0, 0, 0, 0, 0, 0, 0, 0,
              'not_attempted', 'not_attempted', 'not_attempted', 'not_attempted', 'not_attempted',
              0, 0, 0, 0, 0, 'none', 0, 0, 0, 0, ?, '', 'unknown')
    `;
...
app.post('/api/start-game', (req, res) => {
  const { registrationNumber, gameStartTime } = req.body;
    ...
    try {
      ...
      const updateTransaction = db.transaction(() => {
        ...
        return updateStmt.run(
          ...
          totalAttempts,
          successfulAttempts,
          Math.round(avgPerformance * 100) / 100,
          levelTimes[1],
          levelTimes[2],
          levelTimes[3],
          levelTimes[4],
          ...
          analyticsData.aiTrainingAccuracy || 0,
...
app.post('/api/update-level-progress', (req, res) => {
  ...
  try {
    ...
    const result = updateStmt.run(
      ...
      analytics?.aiTrainingAccuracy || 0,
  ...
  try {
    const query = `
      SELECT registration_number, student_name, final_score, time_remaining,
             ...
             firewall_threats_blocked, firewall_safe_allowed, ai_training_accuracy,
    ...
    res.json({ 
      ...
      user: {
        ...
        analytics: {
          ...
          aiTrainingAccuracy: user.ai_training_accuracy,
  ...
  try {
    const statsQuery = `
      SELECT 
        ...
        AVG(average_level_performance) as average_performance,
